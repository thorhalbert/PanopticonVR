...
namespace Confluent.Kafka
{
    ...
    internal class Producer<TKey, TValue> : IProducer<TKey, TValue>, IClient
    {
        ...
        // IL2CPP can't have the callback call a class variable.  Maybe we can tuck the instance away in opaque and make a shim?  
        // Can't find yet how we set it above
        ...
        {
            // Now we can at least get this callback since it's static now, but can't do the various reporting
            // back into the class instance unless we can figure out the opaque thing.
        }

        private void DeliveryReportCallbackImplOriginal(IntPtr rk, IntPtr rkmessage, IntPtr opaque)
        {
            // Ensure registered handlers are never called as a side-effect of Dispose/Finalize (prevents deadlocks in common scenarios).
            if (ownedKafkaHandle.IsClosed) { return; }
        ...
        internal Producer(ProducerBuilder<TKey, TValue> builder)
        {
            ...
            // IL2CPP really doesn't like this
